---
title: "Dr. Choan-Seng Song (宋泉盛)"
type: person
tags:
  - 
verification_status: published
featured: true
last_reviewed: 2026-07-17
---
# Dr. Choan-Seng Song (宋泉盛)

**Dr. Choan-Seng Song (宋泉盛, Song Choan-seng)** is a major Taiwanese theologian whose contextual concerns resonate with diaspora identity, mission, and historical memory themes in toward-a-community-of-hope.

## Identity Snapshot
- Era: 20th–21st century theological influence
- Geography: Taiwan and global theological discourse
- Core roles: theologian, contextual interpreter, church thought leader

## Testaments / Voice

### Testament 1 (Attributed Summary)
Dr. Song is referenced as a theological horizon figure for interpreting migration, culture, and mission tensions addressed in this source cluster.
- Date/Period: retrospective reference context
- Context: theological framing layer
- Source: toward-a-community-of-hope related pages
- Source confidence C

### Testament 2 (Attributed Summary)
His influence aligns with postcolonial and contextual-theology trajectories used by later interpreters of Taiwanese church history.
- Date/Period: historiographical interpretation layer
- Context: conceptual synthesis
- Source: source-cluster synthesis
- Source confidence C

## Historical Context
Within this cluster, Dr. Song is treated as an intellectual reference rather than a primary narrator of NTPC institutional events.

## Contributions and Legacy
1. Offers a contextual-theological lens for diaspora identity and mission debates. — Source: cluster interpretation (C)
2. Supports cross-linking between Taiwanese theology and North American church-history memory work. — Source: cluster synthesis (C)


## Network

Pages that link to **choan-seng-song** (choan-seng-song):


### People (1)

1. [[people/index.md|index]]

### Organizations (1)

1. [[organizations/index.md|index]]

## Source Notes and Confidence

### Source Confidence Legend
- Source confidence A: direct quotations or primary-source textual attributions from the currently attached source set.
- Source confidence B: near-contemporary attributed references or secondary materials closely tied to the event/person.
- Source confidence C: compiled contextual interpretation layers that require further primary-source confirmation.

- Medium confidence: significance as theological reference in this cluster (C)
- Needs verification: direct quotation from source materials currently attached to this cluster

## Name Variants / Disambiguation
- English: Choan-Seng Song
- Chinese: 宋泉盛
- Romanization variant: Song Choan-seng

## Contribution Invitation
Use the shared intake process at Echopedia Community Contributions Hub and include:
- Person/page name
- Your relationship to the person/event
- Memory/quote/correction text
- Date or estimated period
- Source type and permission status

## Related Pages
- toward-a-community-of-hope
- [[organizations/presbyterian-church-in-taiwan|Presbyterian Church in Taiwan (台灣基督長老教會)]]
- Echopedia Person Recordation Framework
