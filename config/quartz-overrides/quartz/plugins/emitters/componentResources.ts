...
('script')[0];
  ...
  // Trailing slash redirect: if URL ends with / and doesn't have a file extension,
  // redirect to the same URL without the trailing slash
  componentResources.beforeDOMLoaded.push(`
    (function() {
      var p = window.location.pathname;
      var baseUrl = "${cfg.baseUrl ?? ""}";
      if (baseUrl && p.indexOf("/" + baseUrl) === 0) {
        p = p.substring(("/" + baseUrl).length);
      }
      if (p.match(/^\\/(.+)\\/$/) && !p.match(/^\\/.+\\.[^/]+\\/$/)) {
        window.location.href = window.location.origin + "/" + "${cfg.baseUrl ?? ""}" + p.replace(/\\/$/, "");
      }
    })();
  `)
}
