...
if ! [[ "$PAR" =~ ^[1-9][0-9]*$ ]]; then
    echo "=== Timeline Builder: skip (peak-hour/freeze par=${PAR:-0}) ==="
    exit 0
fi
export PYTHON SCRIPT_DIR
export OMP_NUM_THREADS=1 MKL_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1
echo "=== Timeline Builder: All TAHS Members (par=$PAR) ==="
echo "Processing ${#SLUGS[@]} members..."

printf '%s\n' "${SLUGS[@]}" | xargs -P "$PAR" -I{} \
    $PYTHON "$SCRIPT_DIR/echopedia-timeline-builder.py" --person {}
