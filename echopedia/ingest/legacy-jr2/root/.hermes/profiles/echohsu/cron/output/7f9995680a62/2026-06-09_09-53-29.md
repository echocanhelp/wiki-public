...
Checked:
- Pulled `/root/wiki-deploy` from `origin/master` before edits.
- Live homepage returned HTTP 200.
- Dependency-light homepage href audit found 23 hrefs:
  - 12 user-facing internal links checked.
  - 0 broken internal links.
  - resource/preconnect links classified as non-content.
- Featured People and Featured Institutional Memory links all returned 200.
...
- Homepage metrics checked against deployed corpus:
  ...
  - Homepage metrics were already accurate.
- Privacy/accuracy smoke check on featured person/navigation pages.

Changed:
- Ran wiki link deduplication: 111 duplicate wiki links removed across 43 pages, then targeted-copied those page changes to deployment.
- Removed unnecessary private/contact details from public person pages:
  - Removed email + location from `willy-pan-潘建宇.md`.
  - Removed spouse/marriage detail from `mingyuan-hsu-許明遠.md`.
- Fixed one markdown trailing-whitespace issue in `toward-a-community-of-hope.md` so `git diff --check` passes.

Validation before push:
- Broken-link scan passed.
- `git diff --check` passed.
- Deploy-faithful Quartz build reproduced locally with Quartz `v4.5.2`: build completed successfully for 437 files.

Pushed:
- Commit: `32966f303fdcd34555b9fd23c1d5b7494697a4eb`
- Message: `Wiki daily audit: fix links and homepage accuracy`

Deployment:
- GitHub Actions run completed successfully for the matching commit.
