...
**Echopedia Homepage Curator Report (2026-06-12 cron run)**

- **Person pages scanned**: 30 valid candidates after aggressive filtering (excluded gstpc-*, echopedia-*, event/, topic/, albert-chapter*, bulletins, indexes, non-person keywords, etc.).
- **Ranking applied**: `sort(key=lambda x: (-lines, -mtime))` on YAML frontmatter `title` + line count + mtime.
- **Top 6 Featured People selected**:
  1. Rev. Mingyuan Hsu (許明遠) — 125 lines
  2. Rev. David Huang (黃德利) — 110 lines
  3. Elder Deng Shuzhen (鄧淑貞長老) — 79 lines
  4. Rev. Pinghsi Liu (劉炳熹) — 75 lines
  5. Willy Pan (潘建宇) — 74 lines (note preserved: “— New TAHS volunteer member”)
  6. Dr. Yunching Yeh Davis (葉芸青) — 74 lines (new entry; replaced Dr. Albert S. Lai)
- **Change detected**: Featured People list differed from previous (Albert S. Lai rotated out). Institutional Memory section left hardcoded per v1 spec.
- **Files updated** (byte-identical):
  - `/root/wiki-public/content/index.md`
  - `/root/wiki-deploy/content/index.md`
- **Git sync**: Committed + pushed to `master` in wiki-deploy (`chore(echopedia): rotate Featured People (daily curator)`).
- **Institutional Memory** (unchanged):
  - Taiwanese American Historical Society…
  - GSTPC Bulletin Historical Signals Index…
  - Toward a Community of Hope…
  - Albert Chapter 1 Audiobook Consent Kit…

Daily rotation complete. No repetition-avoidance log yet (v1).
