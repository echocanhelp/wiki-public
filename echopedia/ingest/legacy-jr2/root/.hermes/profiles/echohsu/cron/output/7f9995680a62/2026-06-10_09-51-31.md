...
Checked:
- Pulled `/root/wiki-deploy` from `origin/master` — already current before edits.
- Live homepage returned HTTP 200.
- Audited homepage links with dependency-light Python URL extraction.
- Classified resource/preconnect/static links as non-content.
- Verified Featured People / Featured Institutional Memory links; all selected live pages returned HTTP 200 after proper percent-encoding for Chinese slugs.
- Checked symlink loops under `/root/wiki-public/content` — none found.
- Ran wiki broken-link scan — no broken wiki links found.
- Ran dedup script — 0 files changed, 0 duplicate links removed.
- Checked deployed corpus metrics:
  - Published pages: 437
  - Person records: 35
  - Publication/archive entries: 337
- Privacy smoke check on featured person/navigation pages — no email/phone/address/family-detail signals found.
- Ran `git diff --check` — clean.
- Reproduced deploy-faithful Quartz build with `jackyzha0/quartz` `v4.5.2` — build succeeded.

Changed:
- Updated homepage metric in both source and deploy content:
  - `Standalone Person Records`: **34 → 35**

Deployed:
- Commit: `37dd29d94dd1fd58b135cea347e317cae8c58576`
...
- `/people`: 200
- `/albert-s-lai`: 200
- `/mingyuan-hsu-許明遠`: 200
- `/david-huang-黃德利`: 200
- `/taiwanese-american-historical-society-台美人歷史協會`: 200
- `/gstpc-bulletin-historical-signals-index`: 200
- `/toward-a-community-of-hope`: 200
- `/albert-chapter1-audiobook-consent-and-recording-kit`: 200
- Live homepage now shows the corrected **35** person-record count.
