...
Checked:
- Pulled `/root/wiki-deploy` from `origin/master` successfully.
- Live homepage returned HTTP 200.
- Extracted homepage links with Python `requests` + regex.
- Validated all user-facing internal homepage links: 12/12 returned HTTP 200.
- Confirmed Featured People and Featured Institutional Memory links return non-404.
- Symlink loop check: no symlinks found under `/root/wiki-public/content`.
- Dedup script: 0 files changed, 0 duplicate links removed.
- Broken wiki-link scan: no broken links found.
- Privacy/accuracy smoke check on featured person/navigation pages: no actionable private-contact/DOB/residence exposure found.
- `git diff --check`: passed.
- Deploy-faithful Quartz v4.5.2 build reproduction: passed, 439 Markdown input files built.

Changed:
- Updated homepage metrics in both:
  - `/root/wiki-public/content/index.md`
  - `/root/wiki-deploy/content/index.md`
- Metrics corrected to:
  - Published Pages: 439
  - Standalone Person Records: 36
  - Publication / Archive Entries: 337

Deployment:
- Commit: `6252f2912cdf58d7c3df64525d5c4ff2530ebaa5`
- GitHub Actions run completed: success.

Live verification:
- Homepage: 200
- Updated metrics visible live: yes
- Sample pages verified 200:
  - `/people`
  - `/toward-a-community-of-hope`
  - `/albert-s-lai`
- Final homepage internal link sweep: all 12 user-facing internal links returned 200.
