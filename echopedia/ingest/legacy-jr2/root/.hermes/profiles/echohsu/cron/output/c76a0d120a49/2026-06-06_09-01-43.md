...
1. `find ... *-*.md` + python glob filter: exclude gstpc-*, echopedia-*, line-*, toward-*, good-shepherd-*, nechopedia-*, and any *society* or *historical-society* basenames.
...
**Echopedia Homepage Curator Report (2026-06-06 cron run)**

- Scanned 54 candidate person pages (aggressive filter applied: excluded gstpc-*, echopedia-*, line-*, toward-*, good-shepherd-*, nechopedia-*, *society*).
- Ranked by `(-line_count, -mtime)`; top 6 selected with frontmatter `title:` parsed where present.
- **Featured People** block updated (order change detected: Deng Shuzhen now precedes Willy Pan per line count 80 > 79; preserved "— New TAHS volunteer member" note exactly).
- Identical targeted patch applied to both `/root/wiki-public/content/index.md` and `/root/wiki-deploy/content/index.md`.
- Institutional Memory section left hardcoded (v1).
- In wiki-deploy: committed + pushed to `master` (remote observed as wiki-public during run).
- Both index.md files are now byte-identical post-update.

**New Featured People list:**
1. Rev. Mingyuan Hsu (許明遠)
2. Rev. David Huang (黃德利)
3. Dr. Albert S. Lai (賴信雄)
4. Elder Deng Shuzhen (鄧淑貞長老)
5. Willy Pan (潘建宇) — New TAHS volunteer member
6. Albert Chapter 1 Audiobook — Consent & Recording Kit

Change committed and deployed. (Next run will be silent unless ranking shifts again.)
