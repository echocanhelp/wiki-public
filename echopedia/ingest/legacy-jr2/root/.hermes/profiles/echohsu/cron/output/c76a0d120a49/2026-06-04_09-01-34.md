...
**Echopedia Homepage Curator Report (2026-06-04 cron run)**

- **Candidates scanned**: 40 person pages after aggressive filtering (excluded all gstpc-*, echopedia-*, line-*, toward-*, good-shepherd-*, nechopedia-*, *society* basenames).
- **Ranking applied**: `sort(key=lambda x: (-lines, -mtime))` on YAML frontmatter `title` + line count + mtime.
- **New top 6 Featured People** (differs from previous; update performed):
  1. Rev. Mingyuan Hsu (許明遠)
  2. Rev. David Huang (黃德利)
  3. Dr. Albert S. Lai (賴信雄)
  4. Willy Pan (潘建宇) — New TAHS volunteer member *(note preserved)*
  5. Elder Deng Shuzhen (鄧淑貞長老)
  6. Albert Chapter 1 Audiobook — Consent & Recording Kit
- **Institutional Memory section**: Unchanged (hardcoded v1).
- **Files updated**: Both `/root/wiki-public/content/index.md` and `/root/wiki-deploy/content/index.md` (byte-identical after patch).
- **Deploy action**: `git add`, conventional commit, `git push origin master` succeeded (new commit 5899cb4).

Featured People list rotated as expected. No repetition-avoidance log yet (v1).
