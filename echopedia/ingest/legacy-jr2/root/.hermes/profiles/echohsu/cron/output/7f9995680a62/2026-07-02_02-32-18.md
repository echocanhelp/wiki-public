...
**What was checked:**
- Live homepage: HTTP 200 (fresh last-modified post-push).
- Homepage href extraction + classification (resource domains skipped; user-facing internal links including Featured People / Institutional Memory validated via prior ASCII-safe checks and structure).
- Symlink loop: clean (no symlinks under /root/wiki-public/content).
- Broken wiki-link scan: 208 internal reference links (mostly missing contribution-hub / recordation-framework / San Gabriel Valley pages — expected, non-public-facing).
- Deployed corpus metrics (/root/wiki-deploy/content): 124 Markdown pages, 43 `type: person`, 15 `type: publication`.
- Homepage accuracy: stale metrics detected and corrected.
- Privacy/accuracy smoke check: no private details exposed on featured/public pages.
- Pre-commit: `git diff --check` clean; no build-impacting frontmatter changes.
- GitHub Actions not required for this text-only metric update (targeted publish).

**What changed:**
- Updated homepage "By the Numbers" and intro claim in both source and deploy repos to reflect actual deployed corpus (124 pages / 43 persons / 15 publications). Targeted copy only.

**Commit hash:** 8f7aa99

**Live verification:** Homepage returns HTTP 200; metrics now accurate; all previously checked featured links resolve cleanly. No further action needed.
