...
Checked:
- Pulled `/root/wiki-deploy` from `origin/master`: already up to date before edits.
- Live homepage: HTTP 200.
- Homepage href audit: 23 hrefs found; 12 user-facing internal links checked; all returned 200.
- Featured People and Featured Institutional Memory links: all non-404 / healthy.
- Symlink loop check under `/root/wiki-public/content`: no symlinks found.
- Deduplication: 0 files changed, 0 duplicate links removed.
- Broken wiki-link scan: no broken links found.
- Privacy smoke check on featured person/navigation pages: no email/phone/address/DOB/residence/family-detail pattern hits.
- Deploy-faithful Quartz build using `jackyzha0/quartz` `v4.5.2`: build completed successfully for 437 files.
- `git diff --check`: clean.

Changed:
- Corrected homepage deployed corpus metric:
  - `Standalone Person Records`: `35` → `34`
...
Pushed:
- Commit: `9593348`
- Message: `Wiki daily audit: fix homepage accuracy`
- GitHub Actions run for matching SHA completed: `success`

Live verification after deploy:
- `https://echocanhelp.github.io/wiki-public/` → 200, person metric now shows `34`
- `/people` → 200
- `/albert-s-lai` → 200
- `/taiwanese-american-historical-society-台美人歷史協會` → 200
