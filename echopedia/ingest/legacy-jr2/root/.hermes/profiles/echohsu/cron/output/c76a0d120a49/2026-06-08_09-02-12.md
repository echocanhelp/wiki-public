...
**Echopedia Homepage Curator Report (2026-06-08 cron run)**

- **Candidates scanned**: 44 valid `*-*.md` files (top-level, after aggressive exclusion of gstpc-*, echopedia-*, event-*, society-*, etc.)
- **Ranking applied**: `sort(key=lambda x: (-lines, -mtime))`
- **New top 6 Featured People** (titles from YAML frontmatter):
  1. Rev. Mingyuan Hsu (許明遠) — 127 lines
  2. Rev. David Huang (黃德利) — 110 lines
  3. Irvine Taiwanese Presbyterian Church (爾灣台灣基督長老教會) — 90 lines (highest recent mtime)
  4. Dr. Albert S. Lai (賴信雄) — 84 lines
  5. Willy Pan (潘建宇) — 79 lines (note preserved: "— New TAHS volunteer member")
  6. Elder Deng Shuzhen (鄧淑貞長老) — 79 lines

- **Change detected**: Featured People list differed from previous (replaced audiobook kit entry + reordered). Institutional Memory section left hardcoded per v1.
- **Actions completed**:
  - Updated `/root/wiki-public/content/index.md`
  - Synced identical update to `/root/wiki-deploy/content/index.md`
  - `git add`, conventional commit, `git push origin master` (succeeded; commit 72f5bbd)

Both index files are now byte-identical with the refreshed rotation. No repetition-avoidance log yet (v1).
