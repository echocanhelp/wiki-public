...
3. Count homepage metrics against the deploy-faithful corpus, not `/root/wiki-public/content` and not blindly every local file in `/root/wiki-deploy/content`:
   - If `/root/wiki-deploy` has untracked Markdown files, exclude them from public homepage metrics unless this audit intentionally stages/publishes those files.
   - For no-new-file daily audits, use the Git-tracked deployment corpus plus the specific modified file(s) being committed; this matches what GitHub Actions will build after push.
   - Metrics to verify: total Markdown input pages, frontmatter `type: person`, frontmatter `type: publication`.
...
When `/root/wiki-deploy` contains unrelated untracked Markdown files, do **not** use the dirty working tree as the build source for metric counts or CI reproduction. Create a clean local clone of `/root/wiki-deploy` (for example `git clone --depth 1 file:///root/wiki-deploy /tmp/wiki-audit-build-src`) and then overlay only the file(s) being committed (for example `content/index.md`). This prevents untracked operational pages from inflating homepage counts or producing a build that GitHub Actions will not actually see after push.
