...
def drive_upload(args):
    ...
    local_path = _Path(args.file).expanduser().resolve()
    if not local_path.exists() or not local_path.is_file():
        print(json.dumps({"status": "error", "error": f"File not found: {local_path}"}, indent=2), file=sys.stderr)
        sys.exit(1)

    body = {"name": args.name or local_path.name}
    if args.parent:
    ...
    actual_size = int(fetched.get("size", 0)) if str(fetched.get("size", "")).isdigit() else None
    if fetched.get("name") != body["name"] or actual_size != expected_size:
        print(json.dumps({
            "status": "verification_failed",
            "expected_name": body["name"],
            "actual_name": fetched.get("name"),
...
def contacts_create(args):
    ...
    fetched = service.people().get(
        resourceName=created["resourceName"],
        personFields="names,emailAddresses,phoneNumbers"
    ).execute()

    names = [n.get("displayName") or n.get("givenName") or n.get("unstructuredName") for n in fetched.get("names", [])]
    emails = [e.get("value", "").lower() for e in fetched.get("emailAddresses", [])]
    phones = [p.get("value", "") for p in fetched.get("phoneNumbers", [])]

    ok = any((args.name or "") in (n or "") for n in names)
    if args.email:
        ok = ok and args.email.lower() in emails
    if args.phone:
        ok = ok and args.phone in phones
    ...
    print(json.dumps({
        ...
        "verified": True,
        "resourceName": created.get("resourceName"),
        "name": names[0] if names else args.name,
        "emails": emails,
        "phones": phones,
    }, indent=2))
...
def sheets_create(args):
    ...
    body = {
        ...
        "sheets": [{"properties": {"title": args.sheet_title}}],
    }
    ...
    if values:
        service.spreadsheets().values().update(
            ...
            range=f"{args.sheet_title}!A1",
            valueInputOption="USER_ENTERED",
            body={"values": values},
        ).execute()

    fetched = service.spreadsheets().get(
        spreadsheetId=created["spreadsheetId"],
        fields="spreadsheetId,spreadsheetUrl,properties.title,sheets.properties.title"
    ).execute()
    ...
    if values:
        ...
        read_range = f"{args.sheet_title}!A1:{_col_letters(col_count)}{row_count}"
        read_back = service.spreadsheets().values().get(
            spreadsheetId=created["spreadsheetId"],
            range=read_range,
        ).execute().get("values", [])

    actual_title = fetched.get("properties", {}).get("title", "")
    sheet_titles = [s.get("properties", {}).get("title", "") for s in fetched.get("sheets", [])]
    ok = actual_title == args.title and args.sheet_title in sheet_titles
    if values:
        ok = ok and read_back == values
...
def docs_create(args):
    ...
    docs = build_service("docs", "v1")
    fetched = docs.documents().get(documentId=created["id"]).execute()
    fetched_text = _extract_doc_text(fetched)
    actual_title = fetched.get("title", "")

    ok = actual_title == args.title
    if args.body:
        ok = ok and args.body in fetched_text
...
def main():
    ...
    p.add_argument("--name", default="", help="Optional remote file name override")
    p.add_argument("--parent", default="", help="Optional parent Drive folder ID")
    ...
    p.add_argument("--sheet-title", default="Sheet1")
    p.add_argument("--values", default="", help="Optional JSON array of arrays to seed at A1")
