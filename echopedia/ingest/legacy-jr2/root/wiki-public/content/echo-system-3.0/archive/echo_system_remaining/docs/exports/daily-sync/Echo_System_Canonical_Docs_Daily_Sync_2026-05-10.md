...
- Generated_at: 2026-05-10T07:15:09.687844-07:00
...
||---|---|---|
|| LINE | EchoHsu | **Live** — primary public-facing community channel. Rich interaction features: Quick Replies, Flex Messages, Carousels, Rich Menus, Buttons. Quota-aware messaging (reply messages quota-efficient; push/multicast/narrowcast/broadcast consume quota heavily). |
...
- `hermes-gateway-echohsu.service` for EchoHsu LINE/SMS ingress
- `echo-autoloop.service` for the autonomous control loop
- LINE bridge (`line_bridge.py`) for LINE webhook and BOT API connectivity
...
- root/default owns Telegram
- orchestrator owns Discord
- echohsu owns LINE (primary, live) and SMS (secondary, active)
...
- Always-on runtime services: `hermes-gateway.service`, `hermes-gateway-orchestrator.service`, `hermes-gateway-echohsu.service`, `echo-autoloop.service`, plus required ingress/bridge infrastructure.
...
- Always-on runtime services: `hermes-gateway.service`, `hermes-gateway-orchestrator.service`, `hermes-gateway-echohsu.service`, `echo-autoloop.service`, `line_bridge.py`
...
- Always-on by default: default/root Telegram gateway, orchestrator Discord gateway, echohsu SMS gateway, `echo-autoloop.service`, and required ingress/bridge infrastructure
...
- Twilio/SMS, Telegram, Discord, and LINE channel connection status
- CPU/RAM/Disk usage
...
- SMS, Telegram, Discord, and future LINE channel status
...
- channel ownership drift across Telegram, Discord, SMS, and future LINE activation
...
- always-on services: `hermes-gateway.service`, `hermes-gateway-orchestrator.service`, `hermes-gateway-echohsu.service`, `echo-autoloop.service`, `line_bridge.py`
- on-demand by default: specialists that do not own an inbound platform and do not require continuous listening
- ownership map: Telegram and Discord are authorized on root/default, orchestrator, and echohsu; echohsu additionally owns SMS plus the API server/LINE bridge surface
...
- orchestrator gateway for Discord operations ingress
- echohsu gateway for LINE (primary) + Twilio/SMS (secondary) public ingress
- LINE bridge (`line_bridge.py`) for LINE webhook and BOT API
- `echo-autoloop.service` for the persistent autonomous control loop
...
5. confirm `echo-autoloop.service` is running
6. confirm EnvironmentOracle and SystemPulse can represent the new state
7. verify no on-demand specialist was accidentally promoted to always-on without policy approval
...
|| Platform | Current Owner | Operational Role |
|| --- | --- | --- |
...
- LINE (primary) and Twilio/SMS (secondary) public conversations enter through EchoHsu
...
### 3.4 Multi-Platform Integration Rules
...
- `line_bridge.py` service is reachable and running as always-on
