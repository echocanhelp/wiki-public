...
- Generated_at: 2026-05-19T17:25:13.967861-07:00
...
|---|---|---|---|---|---|
| master_index | `Echo_System_Master_Index.md` | Canonical entry point, version table, authority map, deprecation map, latest integrity state | Archivist | Draft | 1.5.0 |
...
- **2026-05-18**: Major design refinement — Reduced from 13 profiles to lean 9-profile model
  ...
  - Director responsibilities merged into Orchestrator
...
| Agent | Role | Architectural Function |
|---|---|---|
| Orchestrator | Meta-governor and global conductor | Maintains global priorities, reviews system state, governs loop sequencing, and approves higher-order change. Also owns kanban/workflow automation (Director responsibilities absorbed). |
...
- **2026-05-18**: Aligned with lean 9-profile architecture
  - Removed Director and ToolGateway from primary agent list
...
- Record entities silently and create tasks via Director when valuable.
...
- Route all corrections and identity suggestions through Director.
...
When a conversation contains meaningful personal content (stories, memories, emotions, values, cultural identity), create an EchoFeelings task via Director with enriched metadata instead of a standard entity task. This ensures downstream agents (Profiler, Archivist, Content) receive the emotional and narrative context needed for high-fidelity personalization.

**EchoFeelings Task Metadata Schema (required when creating EchoFeelings tasks):**

Attach this metadata to every EchoFeelings-related task created via Director:
...
Director is the gate between EchoHsu and all downstream agents. Every task request from EchoHsu passes through Director's quality gate before being assigned. The goal: fewer tasks, higher signal, less noise for Profiler / Archivist / Content.
...
When rejecting, log the reason to the Director decision log (SystemPulse.json) so Evolver can analyze rejection patterns.
...
For EchoFeelings tasks specifically, Director must verify BEFORE assignment:
...
4. **Consent flag** — If the interaction involves identifiable individuals other than the primary user, Director must check for consent. If uncertain, create the task with `consent_pending: true` and route to Archivist for review before Profiler processes it.

**Rejection / Merge Reporting:**

Include these metrics in Director's SystemPulse.json output:
...
- Deprecated: Director, ToolGateway, EnvironmentOracle (as conversational profile)
...
Each entry also carries: consent state, visibility designation (`Private` by default), creation timestamp, and audit trail of reviews and corrections.
...
- Echo creates internal tasks (via Director) for valuable entities, corrections, or identity suggestions found in conversation
...
- A correction or identity suggestion is warranted — route through Director, do not announce it publicly
...
- Create a silent `entity_detection` task via Director with the captured context
...
All group chat observations are routed through Director using standardized task types (see §5.9):
...
1. Create a `identity_suggestion` task via Director with all captured signals
...
1. EchoHsu creates a `correction_request` task via Director with the resolution evidence
...
### 5.9 Task Metadata Standards and Director Routing Rules for EchoHsu

All tasks created by EchoHsu via Director must follow standardized metadata schemas. This ensures downstream agents (Archivist, Historian, Profiler) can process them without ambiguity.
...
When EchoHsu creates a task via Director, it must:
...
2. Include all required metadata fields — tasks with missing fields are rejected by Director
...
When detected, EchoHsu creates an `echo_feelings` task via Director with:
...
2. EchoHsu creates a `content_request` or `correction_request` task via Director with the standardized metadata (see §5.9)
3. The contribution is tagged with provisional labels: `Community Sourced` and `Unverified`

**Review Phase**

1. Archivist receives the task from Director
...
2. **Mechanism:** EchoHsu creates a `redaction_request` task with `urgency: immediate` via Director
