...
# SOURCE: Echo_System_Morning_Briefing_Protocol.md
...
### 2.1A Formal Runtime Baseline

SystemPulse is not just a status feed; it is the daily read-back surface for the formal runtime baseline.
...
- On-demand by default: specialist gateways that do not own an inbound platform and do not need continuous listening
- Ownership map: Telegram and Discord are authorized on root/default, orchestrator, and echohsu; echohsu additionally owns SMS and the API server/LINE bridge surface
...
- Anti-false-positive rule: `channel_directory.json` may help with target resolution but is not proof of current platform ownership
...
```json
{
  ...
  "runtime_baseline": {
    ...
    "on_demand_profiles": ["historian", "archivist", "content", "videoforge", "profiler"],
    "channel_owners": {
      "telegram": "root/default",
      "discord": "orchestrator",
      "sms": "echohsu"
    },
    "secret_redaction_required_for_always_on_public_ops_gateways": true,
    "ownership_verification_sources": ["fresh gateway logs", "gateway_state.json", "service status"],
    "channel_directory_is_live_ownership_proof": false
  },
...
### 3.3 Baseline Compliance Requirement

The Morning Briefing must explicitly report any drift from the formal runtime baseline, including:
- missing or unexpectedly started gateway services
- ownership drift across Telegram, Discord, SMS, or future LINE deployment
...
If no drift is found, the briefing should say so plainly rather than imply it indirectly.
...
### 6.1 Verified Executor / Receipt Flow (Phase 1 + Phase 2 Runtime Upgrade)

As of the live Phase 1 + Phase 2 runtime upgrade, the autonomous loop no longer treats downstream LLM prose alone as proof of external execution. For the `historian`, `archivist`, `content`, `videoforge`, and `echohsu` stages, the runtime now follows this order:
...
4. A deterministic executor performs the safe side effect, packaging step, or explicit block/hold decision
...
Current verified side effects / package stages:
...
- VideoForge: render-plan gate recorded as either blocked or packaged from `videoforge.plan.json`; if render-ready becomes true, the daemon writes `runtime/render_jobs/YYYY-MM-DD/videoforge_package.json` and verifies it by read-back
- EchoHsu: final recipient-facing delivery package staged to `runtime/delivery_log/YYYY-MM-DD/delivery_package.json` with JSON round-trip verification; Phase 2 still does not claim outbound send success

Live inference-path fix and verification note:
- The original live `--once --force-all` hang was traced to profile inference, not the executor code path.
- The daemon now invokes autonomous specialist stages in one-shot mode with explicit `openai-codex` / `gpt-5.4` overrides, which resolved the blocking behavior seen at `sentinel`.
- After this fix, a full one-shot live run completed successfully through `echohsu`, and the runtime produced real `historian`, `archivist`, `content`, `videoforge`, and `echohsu` sidecars/receipts under `runtime/stage_outputs/2026-05-09/`.
- In that verification run, `videoforge.receipt.json` correctly recorded a blocked state because upstream evidence still did not justify a render claim, while `echohsu.receipt.json` correctly recorded staged-only delivery packaging without hallucinating an outbound send.
...
# SOURCE: Echo_System_Remaining_Agent_Prompts.md
...
- Formal baseline registry in EnvironmentOracle/SystemPulse: always-on services, on-demand specialists, channel ownership map, and verification-source precedence
...
# SOURCE: Echo_System_Autonomous_Loop_Executor_Receipt_Architecture.md

# Echo System 3.0 — Autonomous Loop Executor / Receipt Architecture

**Version:** 3.2 Phase 1 + Phase 2  
**Date:** May 2026  
**Status:** Implemented in runtime and live-verified  
**Owner:** Orchestrator
...
The runtime now closes that gap through a strict:

planner -> structured JSON -> deterministic executor -> receipt -> Pulse update

flow.

This is the daemon-level enforcement of Layer 4 verification: no stage may claim execution success unless the runtime can read back proof.
...
- actual MP4 rendering is not yet executed by the daemon
- actual outbound message delivery is not yet executed by the daemon
- Phase 2 adds verified gating and staging, not fake completion claims

---

## 4. New runtime contract

For executor-enabled stages, the loop now executes in this order:
...
- `videoforge.plan.json` = machine-readable render-readiness gate and packaging plan
- `echohsu.delivery.json` = machine-readable final delivery package plan
...
- `blocked` = executor correctly refused to claim success because prerequisites were not met
- `failed` = executor attempted work but verification failed

A blocked receipt is not a hallucinated failure. It is a verified refusal to over-claim.
...
Executor behavior:
- read `videoforge.plan.json`
- if `render_ready=false`, write a blocked receipt with planner-gate evidence
- if `render_ready=true`, write:
  - `runtime/render_jobs/YYYY-MM-DD/videoforge_package.json`
- verify by JSON round-trip read-back

Verified outputs:
- `videoforge.receipt.json`
- optional `videoforge_package.json` path in receipt when render-ready

Phase 2 limitation:
- no MP4 render or Google Drive video upload is claimed yet

### EchoHsu

Executor behavior:
- read `echohsu.delivery.json`
- if `delivery_ready=false`, write a blocked receipt
- if `delivery_ready=true`, write:
  - `runtime/delivery_log/YYYY-MM-DD/delivery_package.json`
- verify by JSON round-trip read-back
- mark the package as `staged_only=true`

Verified outputs:
- `echohsu.receipt.json`
- `delivery_package.json` path in receipt

Phase 2 limitation:
- no outbound SMS/LINE/Telegram/Discord delivery is claimed yet
...
## 9. Inference-path fix discovered during live debugging

During live verification, a full `--once --force-all` run initially appeared to hang at `sentinel`.

Root cause found:
- the stall was upstream of the executor logic
- `gather_snapshot()` completed quickly
- the problematic path was profile inference on real autonomous-loop prompts
- the same sentinel prompt completed normally when forced through `openai-codex` with `gpt-5.4`

Runtime fix applied:
- autonomous specialist invocation now uses Hermes one-shot mode
- explicit overrides are applied in the daemon:
  - provider: `openai-codex`
  - model: `gpt-5.4`

Outcome:
- the blocking behavior was resolved
- a full live one-shot run completed through all stages

---

## 10. Validation completed

Verified during implementation and follow-up debugging:
...
- unit tests for VideoForge package staging
- unit tests for EchoHsu delivery-package staging
...
- verified Google Drive upload of the earlier architecture export bundle
- successful full `--once --force-all` one-shot runtime execution after the inference-path fix

Successful live one-shot completion included:
- sentinel
- healer
- evolver
- orchestrator
...
Observed live Phase 2 behavior on `2026-05-09`:
- `videoforge.receipt.json` recorded `blocked`
- `echohsu.receipt.json` recorded `executed`
- `delivery_package.json` was written and verified by read-back
- `SystemPulse.json` reflected receipt-backed status for both stages

This is correct behavior: the runtime refused to over-claim a render, but it did verify the staged delivery package.
...
### Phase 3
- add real VideoForge render execution
- verify final media file existence, size, and duration
- add verified Google Drive upload handles for final videos

### Phase 4
- add real EchoHsu outbound channel delivery
- verify message-handle confirmation from the channel provider
- only then allow delivery success claims

### Future hardening
- add explicit upstream receipt consumption so later stages must read earlier gate/receipt states before proceeding
- add higher-level daily execution ledgers summarizing all stage receipts
