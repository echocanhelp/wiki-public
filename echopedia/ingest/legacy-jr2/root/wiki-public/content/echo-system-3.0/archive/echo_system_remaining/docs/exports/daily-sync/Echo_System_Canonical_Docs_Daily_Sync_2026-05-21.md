...
- Generated_at: 2026-05-21T07:15:44.737801-07:00
...
Version: 1.6.0
Status: Updated — Canonical Docs Sync (Echopedia Redesign Section 10)
Owner: Archivist (Documentation Integrity) with Orchestrator approval for structural changes
Last Updated: 2026-05-20 PT
...
- HTTP mux routes: dashboard → `8080`, SMS/Twilio → `8081`, LINE bridge → `8082`.
...
|---|---|---|---|---|---|
| master_index | `Echo_System_Master_Index.md` | Canonical entry point, version table, authority map, deprecation map, latest integrity state | Archivist | Updated | 1.6.0 |
| vision_architecture | `Echo_System_Vision_Architecture.md` | High-level system mission, architecture, deployment shape, model topology, initialization baseline | Orchestrator | Draft | 1.3.0 |
| knowledge_core | `Echo_System_Knowledge_Core.md` | Knowledge model, graph schema, archival and verification logic, knowledge-facing agent responsibilities | Archivist + Historian | Draft | 1.4.1 |
| agent_prompts | `Echo_System_Agent_Prompts.md` | Single canonical prompt file for all 12 primary conversational agents | Orchestrator | Draft | 1.4.0-draft |
| operations_guide | `Echo_System_Operations_Guide.md` | Deployment, operations, extension, recovery, maintenance, and operator procedures | ToolGateway + Orchestrator | Draft | 1.2.4 |
...
- the 13-profile operating model with restored media pipeline (with EnvironmentOracle as data artifact)
...
Echo System 3.0 operates through a four-layer 13-profile runtime architecture for live autonomous behavior.

### 4.1 Four-Layer Runtime Agent Architecture

Echo System 3.0 operates through 13 named agents organized across 4 functional layers.
...
| AudioForge | Music, SFX & ambient audio generator | Generates music, sound effects, and ambient audio using grok-imagine-audio. |
| Voice | Text-to-speech narration | Generates narration, voiceovers, and character dialogue using grok-tts-1. |
| Vision | Visual quality assurance gate | Performs visual QA on all media assets before delivery using grok-2-vision-latest. |
...
### 6.1 Primary Inference Lane

The following 13 profiles run on frontier paid inference via `xai-oauth` through `https://api.x.ai/v1`:

**Text profiles (grok-4.3):**
- `default`
- `archivist`
- `echohsu`
- `evolver`
...
**Media profiles (grok-imagine model family):**
- `content` → `grok-4.3` (text generation for scripts/narratives)
- `videoforge` → `grok-imagine-video` (video generation)
- `audioforge` → `grok-imagine-audio` (audio/music generation)
- `voice` → `grok-tts-1` (text-to-speech)
- `vision` → `grok-2-vision-latest` (visual QA)
...
### 6.2 Model Routing (updated 2026-05-19)

All 13 profiles run on `xai-oauth` via `https://api.x.ai/v1`. Local vLLM is demoted to fallback-only lane.

**Text lane** — 9 profiles (`default`, `archivist`, `echohsu`, `evolver`, `healer`, `historian`, `orchestrator`, `profiler`, `sentinel`) → `grok-4.3`

**Media lane** — 5 profiles → grok-imagine model family:
- `content` → `grok-4.3`
...
**Fallback lane** — Local vLLM at `http://192.168.7.1:8001/v1` with `Qwen/Qwen3.6-27B-FP8` is available as fallback when xai-oauth is unavailable.
...
- `hermes-gateway-echohsu.service` for EchoHsu Discord + Telegram + SMS + API-server ingress
...
- echohsu owns Discord + Telegram + LINE (primary, live via API-server/bridge) + SMS (secondary, active) + `api_server` bridge surface as the public-ingress layer
...
The system is an Orchestrator-governed 13-profile autonomous knowledge engine for Taiwanese American historical preservation, verification, and storytelling.
...
Owner: Orchestrator
Canonical Role: Single authoritative prompt file for all 13 primary profiles in Echo System 3.0
...
- **2026-05-18**: Aligned with lean 9-profile architecture
  - Removed Orchestrator (formerly Orchestrator) and ToolGateway from primary agent list
...
This is the single canonical prompt file for all 9 primary profiles in Echo System 3.0.
...
This file governs the prompts for the 13-profile system:
- Sentinel
- Healer
- Evolver
- EnvironmentOracle
...
| Content | Narrative and script engine (image generation: grok-imagine-image-quality) | Content production |
| VideoForge | Video generation and packaging studio (grok-imagine-video) | Media production |
| AudioForge | Music, SFX, and ambient audio generation (grok-imagine-audio) | Media production |
| Voice | Text-to-speech narration and voiceover (grok-tts-1) | Media production |
| Vision | Visual quality assurance and verification (grok-2-vision-latest) | Quality assurance |
...
- All profiles run on `xai-oauth` via `https://api.x.ai/v1`.
- Text profiles: `grok-4.3` (orchestrator, echohsu, sentinel, healer, evolver, archivist, historian, profiler).
- Media profiles: `content` (grok-imagine-image-quality), `videoforge` (grok-imagine-video), `audioforge` (grok-imagine-audio), `voice` (grok-tts-1), `vision` (grok-2-vision-latest).
- Local vLLM lane (Qwen/Qwen3.6-27B-FP8 at `http://192.168.7.1:8001/v1`) is now a fallback-only lane, not used by media profiles.
...
- Primary reasoning lane: 9 profiles (`default`, `archivist`, `echohsu`, `evolver`, `healer`, `historian`, `orchestrator`, `profiler`, `sentinel`) on `xai-oauth/grok-4.3` via `https://api.x.ai/v1`
- Local specialist lanes: `content`, `videoforge` on local vLLM at `http://192.168.7.1:8001/v1` with `Qwen/Qwen3.6-27B-FP8` (vLLM currently offline)
...
- Always-on by default: default/root Telegram gateway, orchestrator Discord + Telegram gateway, echohsu Discord + SMS + API-server gateway, `hermes-line-bridge-echohsu.service`, the persistent autonomous loop process `python3 /root/echo_system/runtime/echo_autonomous_loop.py`, and required ingress/bridge infrastructure
...
If you detect a pattern that could become a problem in <24h, create a proactive task in Orchestrator (formerly Orchestrator) for Evolver to analyze.
```
...
- Record entities silently and create tasks via Orchestrator (formerly Orchestrator) when valuable.
...
- Route all corrections and identity suggestions through Orchestrator (formerly Orchestrator).
...
When a conversation contains meaningful personal content (stories, memories, emotions, values, cultural identity), create an EchoFeelings task via Orchestrator (formerly Orchestrator) with enriched metadata instead of a standard entity task. This ensures downstream agents (Profiler, Archivist, Content) receive the emotional and narrative context needed for high-fidelity personalization.

**EchoFeelings Task Metadata Schema (required when creating EchoFeelings tasks):**

Attach this metadata to every EchoFeelings-related task created via Orchestrator (formerly Orchestrator):
...
### Controlled Wikification + User Orchestrator (formerly Orchestrator)y Rules
...
**2. User Orchestrator (formerly Orchestrator)y Protocol**
...
- Orchestrator (formerly Orchestrator)y structure is created on first meaningful interaction (see rule 3).
...
Real-time Trigger: Video request from EchoHsu or Orchestrator (formerly Orchestrator) task
...
5. 5:00 AM — Review proposals, approve safe ones, implement via Orchestrator (formerly Orchestrator)
...
## 16. Orchestrator (formerly Orchestrator) — Kanban + Workflow Automation
...
```
You are Orchestrator (formerly Orchestrator), the Kanban master and workflow automation engine of the Echo System.
...
Example Task Flow:
EchoHsu detects new entity → creates task "Link Lin Mei-Ling to graph + create wiki page" (tags: entity-linking, archivist, high-priority) → Orchestrator (formerly Orchestrator) assigns to Archivist + Profiler in parallel

Output (to SystemPulse.json):
{
  "agent": "Orchestrator (formerly Orchestrator)",
...
Orchestrator (formerly Orchestrator) is the gate between EchoHsu and all downstream agents. Every task request from EchoHsu passes through Orchestrator (formerly Orchestrator)'s quality gate before being assigned. The goal: fewer tasks, higher signal, less noise for Profiler / Archivist / Content.
...
When rejecting, log the reason to the Orchestrator (formerly Orchestrator) decision log (SystemPulse.json) so Evolver can analyze rejection patterns.
...
For EchoFeelings tasks specifically, Orchestrator (formerly Orchestrator) must verify BEFORE assignment:
...
4. **Consent flag** — If the interaction involves identifiable individuals other than the primary user, Orchestrator (formerly Orchestrator) must check for consent. If uncertain, create the task with `consent_pending: true` and route to Archivist for review before Profiler processes it.

**Rejection / Merge Reporting:**

Include these metrics in Orchestrator (formerly Orchestrator)'s SystemPulse.json output:
...
- Primary inference: 9 profiles on `xai-oauth/grok-4.3` via `https://api.x.ai/v1`
- Local specialist inference: `content`, `videoforge` on `http://192.168.7.1:8001/v1` with `Qwen/Qwen3.6-27B-FP8` (vLLM currently offline)
...
5. 5:00 AM — Orchestrator reviews proposals, approves safe ones, implements via Orchestrator (formerly Orchestrator)
...
- `Orchestrator (formerly Orchestrator)` — For task creation and parallel execution
...
- 1.1.0 (2026-05-11) — Updated VideoForge and Orchestrator (formerly Orchestrator) constraints for Safe Mode, resource-gated media execution, and mandatory receipt-backed heavy-task reporting.
...
## Deployment Reality Update (v1.5.0)

All agents must be aware of the current lean architecture:

- Active profiles: Orchestrator, EchoHsu, Archivist, Historian, Profiler, Sentinel, Healer, Evolver (9 total)
- Primary inference: grok-4.3 via xAI for governance and public-facing agents
- Specialist fallback: Local Qwen/Qwen3.6-27B-FP8
- Deprecated: Orchestrator (formerly Orchestrator), ToolGateway, EnvironmentOracle (as conversational profile)
- EnvironmentOracle now functions as a structured data artifact + MCP tools

Every agent prompt should reflect this updated deployment state.
...
- echohsu gateway for Discord + LINE-via-API-server bridge (primary) + Twilio/SMS (secondary) public ingress, with Telegram also verified live after the cleanup pass
...
- Discord, LINE (primary, via EchoHsu API-server bridge) and Twilio/SMS (secondary) public conversations enter through EchoHsu
...
Confirmed identity links are stored in the private wiki with this structure:
```
LINE ID (hashed): [hash]
Wiki Entity: [entity name]
Confidence: Confirmed
Confirmation Date: [date]
Confirmation Method: [self-confirmed / community-confirmed / multi-signal]
```
