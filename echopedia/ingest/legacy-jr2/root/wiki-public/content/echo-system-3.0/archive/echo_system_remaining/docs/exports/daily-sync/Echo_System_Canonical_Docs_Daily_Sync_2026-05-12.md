...
- Generated_at: 2026-05-12T07:26:16.202111-07:00
...
- local `hermes_http_mux.py` on `8079` routes dashboard traffic to `8080`, MCP traffic to `8090`, SMS/Twilio traffic to `8081`, and LINE bridge traffic to `8082`
...
- consumer grok.com currently presents an OAuth-only MCP setup flow; any Grok-web compatibility layer must therefore be deployed as a **separate shim service/hostname** and must not replace the existing mux or Supergateway path
- `hermes-mcp-bridge.service` (Supergateway on 8090) is active and healthy as of 2026-05-12; configured with `--sessionTimeout 300000`, memory cgroup limits (`MemoryMax=512M`), and restart rate limiting to prevent the unbounded memory growth observed in the 2026-05-12 incident (see Operations Guide §6.8, Runtime doc §5.1.1)
...
- 1.1.1 (2026-05-12) — Added MCP bridge service health note: `hermes-mcp-bridge.service` active and healthy with session timeout, memory cgroups, and restart rate limiting. Cross-ref: Operations Guide §6.8, Runtime doc §5.1.1.
...
- public ngrok traffic currently enters through `https://bucked-diabetes-shucking.ngrok-free.dev` -> local mux `127.0.0.1:8079` -> dashboard `8080`, MCP `8090`, SMS `8081`, LINE bridge `8082`
- any Grok-web OAuth compatibility layer must be added as a separate shim hostname/port forwarding upstream to local `8090`, not by mutating the existing mux path
...
- Channel ownership baseline: Telegram and Discord are authorized on root/default, orchestrator, and echohsu; echohsu additionally owns SMS and the API server/LINE bridge surface.
...
- Public control-plane topology: active ngrok hostname `https://bucked-diabetes-shucking.ngrok-free.dev` -> local mux `127.0.0.1:8079` -> dashboard `8080`, MCP `8090`, SMS `8081`, LINE bridge `8082`.
- Grok-web compatibility rule: if consumer grok.com requires OAuth for MCP attachment, use a separate OAuth shim on a new hostname/port forwarding upstream to local `127.0.0.1:8090/mcp`; do not replace the existing mux path.
...
- the mux forwards dashboard traffic to `127.0.0.1:8080`, MCP traffic to `127.0.0.1:8090`, SMS/Twilio traffic to `127.0.0.1:8081`, and LINE bridge traffic to `127.0.0.1:8082`
...
- `ngrok-mcp.service` is present but inactive, so the muxed hostname is the active public MCP surface today
- Grok-web OAuth compatibility, if deployed, must exist as a **parallel shim** on a new port/hostname forwarding upstream to local `127.0.0.1:8090/mcp`
- that shim must not replace the existing public mux or alter the SMS/LINE/dashboard routing contract
- `hermes-mcp-bridge.service` (Supergateway on port 8090) is required for the public `/mcp` endpoint and Grok OAuth shim; it runs with `--sessionTimeout 300000`, memory cgroup limits (`MemoryMax=512M`), and `Restart=on-failure` with rate limiting (`StartLimitIntervalSec=60`, `StartLimitBurst=3`) to prevent the unbounded memory growth observed in the 2026-05-12 incident (see Operations Guide §6.8)
...
- 1.1.1 (2026-05-12) — Added MCP bridge service health note (§5.1.1): `hermes-mcp-bridge.service` configuration (session timeout, memory cgroups, restart policy) to prevent unbounded memory growth. Cross-ref: Operations Guide §6.8.
...
Canonical Role: Single authoritative operator handbook for deployment, startup, day-to-day operations, recovery, extension, security enforcement, and long-term stewardship of Echo System 3.0
...
- on-demand by default: specialists that do not own an inbound platform and do not require continuous listening
- ownership authorization map: Telegram and Discord are authorized on root/default, orchestrator, and echohsu; echohsu additionally owns SMS plus the API server/LINE bridge surface
...
- public ingress topology: active ngrok `hermes-public` hostname -> local `127.0.0.1:8079` mux -> dashboard `8080`, MCP `8090`, SMS `8081`, LINE bridge `8082`
...
- mux routes `/mcp` and `/healthz` -> local Supergateway on `127.0.0.1:8090`
...
- consumer grok.com currently presents an OAuth-only MCP form
- therefore any Grok-web compatibility layer must be deployed as a **parallel OAuth shim** on a new port and new public hostname
- that shim must forward upstream to local `127.0.0.1:8090/mcp`, not to the existing public ngrok hostname, so that dashboard, SMS, LINE, and current MCP routing remain unchanged
...
- channel ownership: Telegram and Discord are authorized on root/default, orchestrator, and echohsu; echohsu additionally owns SMS plus the API server/LINE bridge surface
...
### 6.8 Incident Record: 2026-05-12 MCP Bridge Memory Leak (Resolved)

Observed pattern:

- `hermes-mcp-bridge.service` consumed increasing memory until OOM/crash
...
- Startup script (`/root/hermes_mcp_bridge_start.sh`): removed `--stateful` flag, added `--sessionTimeout 300000`
- Systemd unit: added `MemoryMax=512M` / `MemoryHigh=384M` cgroup limits, `Restart=on-failure`, `StartLimitIntervalSec=60` / `StartLimitBurst=3`
- Service restarted clean: 40M memory usage, well within limits
- Health endpoint (`/healthz`) returns "ok"; MCP endpoint accepts POST/SSE connections

Architecture note:

- This service is required for the public `/mcp` endpoint and the Grok OAuth shim
- It fronts the Hermes MCP server via Supergateway on port 8090
- The local mux (`hermes_http_mux.py` on 8079) routes public `/mcp` traffic to it

Prevention:

- Ongoing memory monitoring via Sentinel/SystemPulse
- Memory cgroup limits prevent runaway from affecting other services
- Session timeout ensures stale sessions are evicted
...
- 1.1.1 (2026-05-12) — Added MCP Bridge memory leak incident (§6.8): root cause (stateful flag + no session timeout), fix (removed --stateful, added --sessionTimeout 300000, memory cgroup limits), service restored to healthy state.
